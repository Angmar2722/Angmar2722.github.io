...
We can clearly see from the image above that after the second hash, we get our trailing zeroes (8 trailing zero bytes which is what we need to pass the last level). Looking at the first hash, b'\xb9\xd7QS5\x93\xac\x10\xcd\xfb{\x8e\x03\xca\xd8\xba\xbcg\xd8\xea\xea\xc0\xa3i\x9b\x82\x85}\xac\xac\x93\x90', we can see that the first two bytes are b'\xb9\xd7'. Ok so lets say that for one of the levels, the server provides us with the random two byte nonce of b'\xb9\xd7'. What this means is that if we provide the rest of the first hash, b'QS5\x93\xac\x10\xcd\xfb{\x8e\x03\xca\xd8\xba\xbcg\xd8\xea\xea\xc0\xa3i\x9b\x82\x85}\xac\xac\x93\x90', as our nonce, the server will append that to b'\xb9\xd7' and then hash that which will produce the second hash, b'\x1d\xbd\x98\x1f\xe6\x98Wv\xb6D\xb1s\xa4\xd08]\xdc\x1a\xa2\xa8)h\x8d\x1e\x00\x00\x00\x00\x00\x00\x00\x00'. Since this second hash has 8 trailing zero bytes, it would pass any level as the last level has 8 trailing non-zero bytes and previous levels have fewer non-zero bytes (assuming the nonce was the correct one provided for this case). 
