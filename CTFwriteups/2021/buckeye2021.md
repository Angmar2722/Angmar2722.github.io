...
In this challenge, we have to provide a number between 256 and 512 bits which passes Fermat's primality test. After that, a random number is generated by creating a 102 byte string `a` which consists of 6 bytes of hello and the hex encoding (96 characters but bytes since encoded) of 48 random bytes which is passed into a `RNG` function. 
