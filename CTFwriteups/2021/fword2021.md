...
The prime `p` along with the values `r` and `s` are returned by the server. Our objective is to predict the 'ticket' (i.e. the p, r and s values) of the word 'Boombastic'. Since the prime p and secret is constant per server session we already have that. Recently, we learned of something known as a <a href="https://en.wikipedia.org/wiki/Gr%C3%B6bner_basis" target="_blank">Gröbner basis</a> where it is defined for ideals in a polynomial ring {\displaystyle R=K[x_{1},\ldots ,x_{n}]}R=K[x_1,\ldots,x_n] over a field K. Although the theory works for any field, most Gröbner basis computations are done either when K is the field of rationals or the integers modulo a prime number as it is in our case. Hence we could use the inbuilt Sage function for the Gröbner basis to recover the secret and hence the r and s values.
