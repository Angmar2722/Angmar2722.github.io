...
I noticed than when multiplying the equation by 2, 2n equals the square root of (i + j). So we can now find j :
...
One problem that I came across is that sometimes I would get a negative value (which probably came because there are two solutions for j (negative and positive) and the negative one happened to be solved for). So my teammate noticed that in these cases where j was negative, the value of i was one less than the modulus (absolute value of j). So we solved for i by multiplying j by -1 (as j was negative) and then subtracting 1. With that we have i and we could then calculate j as j equals the equation below (and we know i and n) :
