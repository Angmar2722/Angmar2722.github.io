...
I realized that you could leak the contents of variables in a stack using the `%x` command (which would output a hexadecimal number). When I inputted `%8$x` and `%9$x`, I got 73465443 and 6e756620 which when converted from hexadecimal to ASCII gave me 
